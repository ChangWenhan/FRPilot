...
- **带宽控制（QoS）**：直接在 frps 服务器上按机器（隧道端口）做 tc 流量整形，不改任何 frpc 配置、不踢会话——**自动均衡模式**按活跃机器数平均分出站/入站预算（不管开多少 SSH 连接都算一台机器，阈值+滞后窗口防抖动），**手动限速模式**为每台机器设置固定额度；未填预算的方向不整形，退出时自动清理全部规则；配置入口在 Web「流量」页（仅管理员），CLI 暂不支持 QoS
...
- **响应式 Web UI**：shadcn 风格设计系统（统一控件高度、严格对齐、长名称省略截断、下拉操作菜单、深浅色主题一键切换），电脑/手机自适应，可"添加到主屏幕"当应用使用
...
| 流量 | 带宽控制（自动均衡 / 手动限速，仅管理员）、带宽流向 Top 图、实时速率、24h 趋势、异常突增标记 |
...
| `qos.*` | 带宽控制：模式 `off`/`auto`/`manual`、出/入站预算 Mbps、活跃判定阈值 KB/s、滞后窗口秒、整形接口、手动限速表 | `off` / 出站 3 Mbps / 1 KB/s / 45s |
| `sessionGraceMinutes` | Web 登录会话滑动窗口（分钟），每次请求自动向后滑动 | `5` |
| `login*` | 防爆破参数（失败次数 / 锁定分钟 / IP 失败上限 / 统计窗口） | 5 / 10 / 15 / 15 |
| `tls.*` | 启用 HTTPS（证书与私钥路径；自签名证书需浏览器放行/导入） | 关闭 |
...
- 浏览器会话使用 **HttpOnly 会话 Cookie**（防 XSS 窃取），不再把 token 存入 localStorage；Web 会话带 **5 分钟滑动窗口**（关闭网页后窗口内再访问无需重新登录，每次请求自动续期）；CLI 通过 `X-FRPilot-Client: cli` 头显式声明后获取固定有效期 Bearer token
...
- 默认 `GOMEMLIMIT=512MB` 软上限 + `GOGC=100`，防止空闲时内存膨胀（可用环境变量覆盖）
- 采集频率：快速指标 30s、慢速模块（安全/定时任务/端口）5min、流量轮询 30s

## 版本记录

- **v0.3.0** — 带宽控制（QoS）：frps 服务器 tc 整形，自动均衡 / 手动限速
- **v0.2.0** — UI 全面统一组件风格；深浅色主题手动切换；Web 登录 5 分钟滑动会话；机器总览并入「机器」页
- **v0.1.0** — 首个正式版本：自动发现、SSH 采集、安全体检、一键清理、AI 诊断、用户与审计

> 热更新不改变版本号：仅替换二进制即可应用修复，见「运维」章节。

## 开发

```bash
# 后端
go build -o frpilot .          # 需要 Go 1.26+
go test ./...
