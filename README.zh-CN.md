...
- **带宽控制（QoS）**：直接在 frps 服务器上按机器（隧道端口）做 tc 流量整形，不改任何 frpc 配置、不踢会话——**自动均衡模式**按活跃机器数平均分出站/入站预算（不管开多少 SSH 连接都算一台机器，阈值+滞后窗口防抖动），**手动限速模式**为每台机器设置固定额度；未填预算的方向不整形，退出时自动清理全部规则
